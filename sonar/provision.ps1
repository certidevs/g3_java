...
# Gate "Curso Java": solo duplicacion (estos proyectos no tienen tests -> sin condicion de cobertura)
Sonar-Post '/api/qualitygates/create' @{ name = $GateName } | Out-Null
try {
  $cur = Invoke-RestMethod "$Sonar/api/qualitygates/show?name=Curso%20Java" -Headers $headers
...
Sonar-Post '/api/qualitygates/create_condition' @{ gateName = $GateName; metric = 'duplicated_lines_density'; op = 'GT'; error = '10' } | Out-Null

# Proyecto + asignar el gate a ESTE proyecto (el gate por defecto sigue siendo el de testing)
Sonar-Post '/api/projects/create' @{ project = $ProjectKey; name = $ProjectKey } | Out-Null
Sonar-Post '/api/qualitygates/select' @{ gateName = $GateName; projectKey = $ProjectKey } | Out-Null
...
Write-Host ""
Write-Host "Listo. Login: admin / $NEWPASS  |  proyecto: $ProjectKey  |  gate: $GateName  |  token en sonar\token.txt"
